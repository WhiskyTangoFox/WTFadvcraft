...
//	}
	 ...
	 */
	public WTFexplosion(World world, Entity exploder, double x, double y, double z, float explosionSize) {
	 ...
	 */
	public WTFexplosion setSource(DamageSource source) {
		this.source = source;
		return this;
	}
	 ...
	 */
	public WTFexplosion setDamage(float amount) {
		damage = amount;
		return this;
	}
	 ...
	 */
	public WTFexplosion setBurnTime(int ticks) {
		burnTime = ticks;
		return this;
	}
	 ...
	 */
	public WTFexplosion setMotionFactor(float amount) {
		motionFactor = amount;
		return this;
	}
	 ...
	 */
	public WTFexplosion restrictExplosionBy(float factor) {
					...
					if (i == 0 || i == MAX_RADIUS - 1 || j == 0 || j == MAX_RADIUS - 1 || k == 0 || k == MAX_RADIUS - 1)
					{
						...
						for (float f2 = 0.3F; f1 > 0.0F; f1 -= f2 * 0.75F)
						{
									...
									(exploder == null || exploder.func_145774_a(this, worldObj, l, i1, j1, block, f1)))
							{
								...
								{
								WTFEventMonitor.Fracture(l, i1, j1, worldObj);
								}
							//TO DO- add an if statement where it vapourizes the blocks if 
							d0 += d3 * (double)f2;
							d1 += d4 * (double)f2;
							d2 += d5 * (double)f2;
						}
	...
	protected void affectEntitiesWithin() {
		float diameter = explosionSize * 4.0F;
